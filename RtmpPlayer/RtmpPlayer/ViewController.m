...
char* rtmpurl="rtmp://localhost:1935/rtmplive/room";//连接的URL
char* flvfilename="me.flv";//读取的flv文件

int ZINIT();//初始化相关
void ZCLEAR();//清除相关


@interface ViewController ()<PCMDelegate,AACEncodeDelegate>
@property (nonatomic ,retain) RecordVideo *recordVideo;
@property (nonatomic ,retain) RecordAudio *recordAudio;
@property (nonatomic ,retain) AudioUnitRecordAudio *audioUnitRecordAudio;
@property (nonatomic ,retain) Converter *converterAAC;

@property (nonnull, strong) NSMutableArray *VideoListArray;
@end
...
//     aaaa = RTMP_Connect(rtmp,NULL);
   ...
   NSString *path = [[NSBundle mainBundle] pathForResource:@"me.flv" ofType:nil];
    const char *p = [path cStringUsingEncoding:[NSString defaultCStringEncoding]];
    fp=fopen(p,"rb");
...
- (void)viewDidLoad {
    ...
    BOOL res = [[RecordManager shareManager]  startRtmpConnect:@"rtmp://192.168.1.186:1935/rtmplive/room"];
    if (!res) return;
...
//    [_recordAudio start];
    ...
    [_recordVideo setLayerDisplay:self.view width:640 height:480];
    
    _converterAAC = [Converter new];
    _converterAAC.delegate = self;
    
    _audioUnitRecordAudio = [AudioUnitRecordAudio new];
    _audioUnitRecordAudio.delegate = self;
    [_audioUnitRecordAudio start];
...
#pragma mark pcmDelegate
- (void)pcmData:(char *)data datalen:(uint32_t)len inputFormat:(AudioStreamBasicDescription)audioFormat{
    [_converterAAC encoderAAC:audioFormat pcmData:data pcmLen:len];

}
#pragma mark aacDelegate

- (void)aacData:(unsigned char *)data datalen:(uint32_t)len{
    NSData *dataByte = [NSData dataWithBytes:data length:len];
    [[RecordManager shareManager] send_rtmp_audio:dataByte andLength:len];
}
